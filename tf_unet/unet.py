...
class Unet(object):
    ...
    def _get_cost(self, logits, cost_name, cost_kwargs):
        ...
        with tf.name_scope("cost"):
            ...
            elif cost_name == "BCEDICE":
                loss = BCEDICE(flat_labels, flat_logits)
            else:
...
class Trainer(object):
    ...
    def __init__(self, net, batch_size=1, verification_batch_size=4, norm_grads=False, optimizer="momentum",
                 opt_kwargs={}):
        self.net = net
    ...
    def _get_optimizer(self, training_iters, global_step):
        if self.optimizer == "momentum":
            learning_rate = self.opt_kwargs.pop("learning_rate", 0.125)  # 0.2
        ...
        #     self.learning_rate_node = tf.Variable(learning_rate, name="learning_rate")
        #
        #     optimizer = tf.train.NadamOptimizer(learning_rate=self.learning_rate_node,
        #                                        **self.opt_kwargs).minimize(self.net.cost,
        #                                                                    global_step=global_step)
    ...
    def store_prediction_val(self, sess, batch_x, batch_y, name, log_path):
        prediction = sess.run(self.net.predicter, feed_dict={self.net.x: batch_x,
                                                             self.net.y: batch_y,
                                                             self.net.keep_prob: 1.})
        test_shape = prediction.shape

        loss = sess.run(self.net.cost, feed_dict={self.net.x: batch_x,
                                                  self.net.y: util.crop_to_shape(batch_y, test_shape),
                                                  self.net.keep_prob: 1.})
        ...
        logging.info("Verification error= {:.1f}%, loss= {:.4f}".format(error_rate(prediction,
                                                                        ...
                                                                        loss))
        log = open(log_path, 'a+')
        log.write("Verification error= {:.1f}%, loss= {:.4f}\n".format(error_rate(prediction,
                                                                                  util.crop_to_shape(batch_y,
                                                                                                     prediction.shape)),
                                                                       loss))
        log.close()
    ...
    def store_prediction_test(self, sess, batch_x, batch_y, name, log_path):
        prediction = sess.run(self.net.predicter, feed_dict={self.net.x: batch_x,
                                                             self.net.y: batch_y,
                                                             self.net.keep_prob: 1.})
        ...
        logging.info("Testing error= {:.1f}%, loss= {:.4f}".format(error_rate(prediction,
                                                                              util.crop_to_shape(batch_y,
                                                                                                 prediction.shape)),
                                                                   loss))
        log = open(log_path, 'a+')
        log.write("Testing error= {:.1f}%, loss= {:.4f}\n".format(error_rate(prediction,
                                                                             util.crop_to_shape(batch_y,
                                                                                                prediction.shape)),
                                                                  loss))
        log.close()
    ...
    def output_epoch_stats(self, epoch, total_loss, training_iters, lr, log_path):
        logging.info(
            "Epoch {:}, Average loss: {:.4f}, learning rate: {:.4f}".format(epoch, (total_loss / training_iters), lr))
        log = open(log_path, 'a+')
        log.write(
            "Epoch {:}, Average loss: {:.4f}, learning rate: {:.4f}\n".format(epoch, (total_loss / training_iters), lr))
        log.close()

    def output_minibatch_stats(self, sess, summary_writer, step, batch_x, batch_y, log_path):
        ...
        logging.info(
            "Iter {:}, Minibatch Loss= {:.4f}, Training Accuracy= {:.4f}, Minibatch error= {:.1f}%".format(step,
                                                                                                           ...
                                                                                                           error_rate(
                                                                                                               ...
                                                                                                               batch_y)))
        log = open(log_path, 'a+')
        log.write("Iter {:}, Minibatch Loss= {:.4f}, Training Accuracy= {:.4f}, Minibatch error= {:.1f}%\n".format(step,
                                                                                                                   loss,
                                                                                                                   acc,
                                                                                                                   error_rate(
                                                                                                                       predictions,
